			...
			if (possessed.movementComponent){
				if(checkTile(possessed.movementComponent.currentPosition) == "roadTile"){
					...
					if(checkTouchingLog() && possessed.typeOfEntity == "player"){ //if touching log
						possessed.movementComponent.currentVelocity.x += checkTouchingLog().logSpeed/10; //move with log ( could be set to the speed of the log, but we'll assume they're always the same speed :) )
					} else { //else die
